class AuthDataProvider {
    ...
    async uiLogin(username, password ) {
        try {
            ...
            const passwordField = await this.page.$x(this.config.loginDomElements.passwordField)
            await passwordField[0].type(password)

            const loginPageBtn = await this.page.$x(this.config.loginDomElements.loginPageBtn)
            ...
            const profileUsername = await this.page.$x(this.config.loginDomElements.profileUsername)
