...
class OrderBookDataProvider {
    ...
    async uiGetSalesPricesByMarket(market) {
        try {
            ...
            const orderBook = [];
            let pureData = (await tableDataScraper(this.config.orderBook.sales.body, this.page)).map(data => persianNumberToEnglish(data));
    ...
    async uiGetBuysPricesByMarket(market) {
        try {
            ...
            const orderBook = [];
            let pureData = (await tableDataScraper(this.config.orderBook.buys.body, this.page)).map(data => persianNumberToEnglish(data));
